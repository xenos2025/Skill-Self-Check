...
function Remove-MaintainerOnlyContent {
    param([string]$Target)

    foreach ($RelativePath in @("tests", "examples\fixtures")) {
        $PathToRemove = Join-Path $Target $RelativePath
    ...
    $ExcludedDirectories = @(
        Get-ChildItem -LiteralPath $Target -Recurse -Force -Directory |
            Where-Object { $_.Name -in @("backup", "backups", "__pycache__") } |
    ...
    Get-ChildItem -LiteralPath $Target -Recurse -Force -File |
        Where-Object { $_.Extension -in @(".bak", ".backup", ".pyc") } |
        Remove-Item -Force
}
...
foreach ($Skill in $Skills) {
    ...
    Remove-MaintainerOnlyContent -Target $Target
...
if (
    ...
    $Skills -contains "skill-ship-safety"
) {
    Write-Host "  full static audit: python `"$HOME\.cursor\skills\skill-self-check\scripts\run_full_audit.py`" path\to\your-skill --out-dir path\outside\the\repo --pretty"
}
